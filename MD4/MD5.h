...
public:
	...
	uint16_t hexdigest_tests() const;
...
private:
	...
	static inline uint4 first_fun(uint4 X, uint4 Y, uint4 Z);
	static inline uint4 second_fun(uint4 X, uint4 Y, uint4 Z);
	static inline uint4 third_fun(uint4 X, uint4 Y, uint4 Z);
public:
